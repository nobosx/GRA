...
For the GuidedRay method, the parameter `--pool_num` is the number of adversarial examples used for guidance (i.e. the parameter $k$ in the paper) and `--augment_method` is the method used in the Adversarial Example Augmentation Framework, the experiment results in this paper are conducted by `mix` method.

To run attacks under other settings, just modify the corresponding parameters.

The supported parameters for `--dataset` and `--model` are: `--dataset cifar10 --model resnet50`, `--dataset cifar100 --model resnet50` and `--dataset imagenet --model densenet121`.

The supported attack methods `--method` are: `RayS`, `GuidedRay`, `HSJA`, `bounce`, `Tangent`, `Sign_OPT_Linf` and `RayST`.

The supported parameters for `--augment_method` are: `gaussian_noise`, `rotation`, `crop`, `color_jitter` and `mix`.
...
### Replacing the Ray Search by Random Selected Search Direction (Section 6.4.2 in the paper)

There is an additional attack method supplied in the repository, namely `GuidedRay-W`, which replace the ray search algorithm by a simlified randomly selected direction update method. We can use the following commands to test `GuidedRay-W` attacks:

```shell
python ./attack_standard.py --dataset cifar10 --model resnet50 --method GuidedRay-W --norm inf --targeted 1 --test_num 1000 --max_query_count 5000 --pool_num 1 --augment_method mix
```


### Testing the Diversity of New Directions (Section 6.4.3 in the paper)
...
### Accuracy of the Fast Test Method (Section 6.4.4 in the paper)

We write additional scripts to test and analyse the accuracy of fast test method in determining the adversarial direction.

The code in `./fast-tester` can use augment method to generate samples for some targets, randomly select adversarial direction or non-adversarial direction and use fast test method to adjust whether it is adversarial direction.
