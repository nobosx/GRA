...
class GuidedRayWAttacker(BasicAttacker):
    ...
    def attack(self, x, y, target_x=None, target_y=None):
        ...
        shape = list(x.shape)
        assert shape[0] == 1, "Attack only supports batch size of 1 right now!"
        dim = np.prod(shape[1:])
        ...
        for i in range(self.max_query_count):
            max_s = int(np.floor(np.log2(dim)))
            s = random.randint(0, max_s)
            block_size = 2 ** s

            max_start = max(0, dim - block_size)
            start = random.randint(0, max_start)
            end = min(dim, start + block_size)
            #print(self.queries)
