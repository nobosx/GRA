...
from utils import build_record_path_defensive
...
def parse_attack_config():
    parser = argparse.ArgumentParser(description="Defensive Adversarial Attack")
    ...
    parser.add_argument('--defense_model', type=str, default='adv_train', help=f"Defense model to be considered. Supported defense models are {SUPPORTED_DEFENSE_MODEL}.")
...
if __name__ == "__main__":
    ...
    defensive_model = DefensiveModel(attack_config['model'], attack_config['dataset'], attack_config['defense_model'])

    # Load adversarial attacker
    attacker = build_concrete_attacker(defensive_model, attack_config)

    # Set attack process
    attack_process = AttackProcess(defensive_model, attackset_loader, attacker, attack_config['targeted'], attack_config['max_query_count'], test_batch_size=attack_config['test_batch_size'], target_pool_num=attack_config['pool_num'], verbose=attack_config['verbose'])
    ...
    # Save attack record
    AttackProcess.save_attack_record(attack_record, build_record_path_defensive(attack_config))
