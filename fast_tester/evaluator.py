...
class AttackProcess(object):
    ...
    def acc_test(self, attack_num: int = 10):
        """
        Mount an adversarial attack and output the results.
        ...
        if self.verbose:
            print(
                "#######################################################################################################################################################################################")
            print("Mounting {} attack against {} model on {} dataset under inf norm. Total test samples: {}.".format(
                "targeted" if self.targeted else "untargeted", self.victim_model.model_name, self.dataset_name, attack_num))
            print(
                "#######################################################################################################################################################################################")
        ...
        for image, label in self.attack_set_loader:
            ...
            # Untargeted attack
            if not self.targeted:
                is_adv, fast_adv, lin_queries = self.attacker.attack(image, label)
            # Targeted attack
            else:
                # Randomly select a target class that is different from the original class
                target_y = torch.randint(0, CLASS_NUM[self.dataset_name], (1,))
                ...
                target_x = random.choices(self.__get_ref_samples_for_one_class(target_y.item()), k=self.target_pool_num)
                target_x = torch.stack(target_x)
                is_adv, fast_adv, lin_queries = self.attacker.attack(image, label, target_x=target_x, target_y=target_y)
            end_t = time()

            attack_record["true_flag"].append(is_adv)
            attack_record["try_flag"].append(fast_adv)
            attack_record["lin_queries"].append(lin_queries)
            if self.verbose:
                print(f"Attacking time consumption: {end_t - start_t} s.")
                print("")
        ...
        if self.verbose:
            if test_count < attack_num:
                print(
                    f"Warning: Running out of qulified test samples. Only {test_count} samples were attacked out of {attack_num} requested.")
            print(
                "#######################################################################################################################################################################################")
            print("All attacks completed!")
            print(
...
class ResultParser(object):
    ...
    @classmethod
    def parse_multi_ASR_with_epsilons_and_queries(cls, attack_record: dict, epsilons: list, max_queries: list, T: int):
        """
        Special for power_bounce attack. Where multiple attack histories are provided.
        """
        success_count = np.zeros((len(epsilons), len(max_queries)), dtype=np.uint32)
        for attack_hist in attack_record["hist"]:
            pos_record = np.zeros(1 + T, dtype=np.uint32)
            for col, query_limit in enumerate(max_queries):
                for i in range(1 + T):
                    while pos_record[i] < len(attack_hist[i]) and attack_hist[i][pos_record[i]][0] <= query_limit:
                        pos_record[i] += 1
                for row, epsilon_metric in enumerate(epsilons):
                    whether_success = False
                    for i in range(1 + T):
                        if pos_record[i] > 0 and attack_hist[i][pos_record[i] - 1][1] < epsilon_metric:
                            whether_success = True
                    if whether_success:
                        success_count[row][col] += 1
        ASR_values = success_count / len(attack_record["hist"])
        return ASR_values
